...
import { AlertTriangle, CalendarDays, CheckCircle2, ChevronDown, Clock3, Plus, Search } from "lucide-react";
...
import { ReminderApprovalsView } from "@/components/collections/reminder-approvals-view";
...
function CollectionsPageBody({ database }: { database: MockDatabase }) {
  ...
  const [view, setView] = useState<View>("all"); const [collectionOpen, setCollectionOpen] = useState(false); const [search, setSearch] = useState(""); const [projectId, setProjectId] = useState("all"); const [customerId, setCustomerId] = useState("all"); const [collectionStatus, setCollectionStatus] = useState("all"); const [villaId, setVillaId] = useState("all"); const [dateFrom, setDateFrom] = useState(""); const [dateTo, setDateTo] = useState("");
