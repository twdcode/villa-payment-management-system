...
const scheduleStageSchema = z.object({ stage: z.string().trim().min(1, "Stage name is required."), gracePeriodDays: z.coerce.number().int("Grace days must be a whole number.").min(0, "Grace days cannot be negative."), deliverables: z.string() });
...
function AddScheduleStageDialog({ defaultGraceDays, onClose, onSave }: { defaultGraceDays: number; onClose: () => void; onSave: (stage: PaymentScheduleDefaultStage) => void }) {
  const [form, setForm] = useState({ stage: "", gracePeriodDays: defaultGraceDays, deliverables: "" });
...
export function WorkspaceSettingsPageClient() {
  ...
  const [error, setError] = useState("");
  const isSuperAdmin = database?.users.find((user) => user.id === "user-vishal")?.role === "super_admin";
  useEffect(() => { void repository.getDatabase().then(setDatabase).catch((reason: unknown) => setError(errorMessage(reason, "Unable to load workspace settings."))); }, []);
  const panel = useMemo(() => {
