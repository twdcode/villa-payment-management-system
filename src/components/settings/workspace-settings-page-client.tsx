...
export function WorkspaceSettingsPageClient() {
  ...
  useEffect(() => { void repository.getDatabase().then(setDatabase).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "Unable to load workspace settings.")); }, []);
  const panel = useMemo(() => {
