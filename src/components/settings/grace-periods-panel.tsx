...
const gracePeriodSchema = z.object({
  ...
  days: z.coerce.number().int("Number of days must be a whole number.").min(0, "Number of days cannot be negative."),
  description: z.string().trim().max(300, "Description cannot exceed 300 characters."),
  useAsDefault: z.boolean(),
});

type GracePeriodForm = z.infer<typeof gracePeriodSchema>;
...
function GracePeriodFormDialog({ period, onClose, onSaved }: { period: GracePeriod | null; onClose: () => void; onSaved: (settings: WorkspaceSettings, message: string) => void }) {
  ...
  const [form, setForm] = useState<GracePeriodForm>({ name: period?.name ?? "", days: period?.days ?? 30, description: period?.description ?? "", useAsDefault: period?.isDefault ?? false });
