...
function ConfiguredVillaProfile({ database, onInterestSaved, onScheduleSaved, projectId, projectName, summary }: { database: MockDatabase; onInterestSaved: () => void; onScheduleSaved: () => void; projectId: string; projectName: string; summary: VillaSummary }) {
  ...
  return <>
    ...
    </div></>}<PaymentScheduleDialog key={`${villa.id}-${scheduleEditorOpen}-${schedules.map((schedule) => schedule.id).join("-")}`} onOpenChange={setScheduleEditorOpen} onSaved={onScheduleSaved} open={scheduleEditorOpen} schedules={schedules} villa={villa} /><InterestTermsDialog key={`${villa.id}-${interestEditorOpen}-${villa.chargeLatePaymentInterest}`} onOpenChange={setInterestEditorOpen} onSaved={onInterestSaved} open={interestEditorOpen} terms={storedTerms} villa={villa} />
  </>;
}
