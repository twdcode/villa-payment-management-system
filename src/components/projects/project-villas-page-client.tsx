        ...
        <tbody className="divide-y">
          {rows.map((row) => {
            ...
            return <tr className="group cursor-pointer transition-colors hover:bg-surface-subtle" key={villaId} onClick={(event) => {
              ...
              <td className="px-5 py-4"><Link className="block rounded-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" href={href}>{number}{row.kind === "configured" && <span className="mt-1 block text-sm font-normal text-muted-foreground">{row.summary.villa.type}</span>}</Link></td>
...
export function ProjectVillasPageClient({ projectId, database }: { projectId: string; database: MockDatabase }) {
  ...
  const visibleRows = useMemo(() => rows.filter((row) => {
    ...
    return (status === "all" || row.summary.villa.operationalStatus === status) && matchesVillaSearch(search, row.summary.villa.number, row.summary.customer?.fullName);
  }), [rows, search, status]);
