...
export function CustomersPageClient({ customerId, database }: { customerId?: string; database: MockDatabase }) {
  return (
    <AppShell active="Customers">
      <CustomersPageBody customerId={customerId} database={database} />
    </AppShell>
  );
}

/** Split so `useToast()` resolves under AppShell's provider — see ProjectsPageBody. */
function CustomersPageBody({ customerId, database }: { customerId?: string; database: MockDatabase }) {
