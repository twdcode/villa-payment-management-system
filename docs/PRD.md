...
- Add collection from a customer profile preselects and locks the current customer and linked villa. It uses the shared Add collection dialog from Collections, including live balances, payment allocation order, and the external document-link field, then saves through the standard collection flow.
...
- The Collections-page **Add collection** action uses the shared responsive payment dialog and lets the user select an active, customer-linked villa with an available payment schedule. The same dialog is used from related Villa and Customer profiles and the Collection-row Record payment action, where the originating customer and villa remain locked. Cancelled villas and villas without a linked customer or payment schedule are excluded from the global selector.
...
- Only Super Admins can add a project payment-schedule stage. **Add stage** opens a responsive form containing a required stage name, non-negative whole-number grace days pre-filled from the workspace default, optional stage deliverables, and Cancel and Save stage actions. Saving the modal adds the validated stage to the current project draft; **Save schedule** persists the complete project default.
